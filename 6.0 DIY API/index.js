...
app.get("/jokes/:id", (req, res) => {
  ...
  console.log(jokes[idJoke-1]);
  res.json(jokes[idJoke-1])
})

//3. GET a jokes by filtering on the joke type
app.get("/filter", (req, res) => {
  const type = req.query.type;
  const listJoke = jokes.filter((joke) => joke.jokeType === type)
  res.json(listJoke);
})
...
app.put("/jokes/:id", (req, res) => {
  ...
  const replaceJoke = jokes.splice(req.params.id - 1, 1, updateJoke);
  res.json(jokes);
})
